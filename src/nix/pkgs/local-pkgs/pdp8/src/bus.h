...
/* This emulator does not allow for non-contiguous memory fields.
   Checking of memory addressing errors is only done when the field
   select registers are changed in KM8E or other I/O device IOT's.

   Note:  Bounds checking on memory addresses is only done when memory fields
   are selected.  Bounds checks are not needed with each memory reference.
*/
